...
signal co2_emitted(co2_dict: Dictionary[Vector2, float])
signal s02_emitted(so2_dict: Dictionary[Vector2, float])
...
func apply_emissions(building, emission_type, amount) -> void:
	...
	var co2_dict: Dictionary[Vector2, float] = {}  
	var so2_dict: Dictionary[Vector2, float] = {}

	# Tile-based pollution
	match emission_type:
		Enums.ResourceType.CO2:
			pollution_falloff(amount, co2_dict, pollution_radius, building_pos)
			co2_emitted.emit(co2_dict)
		Enums.ResourceType.S02:
			pollution_falloff(amount, so2_dict, pollution_radius, building_pos)
			s02_emitted.emit(so2_dict)
		_:
			pass
func pollution_falloff(amount, pollution_dict, pollution_radius, building_pos):
	for x in range(-pollution_radius, pollution_radius + 1):
		for y in range(-pollution_radius, pollution_radius + 1):
			...
			pollution_dict.set(tile_pos, amount_to_set)
