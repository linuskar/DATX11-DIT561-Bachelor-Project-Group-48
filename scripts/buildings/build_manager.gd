...
func _process(_delta) -> void:
	match StateManager.state:
		...
		## TODO: fix tile code in layers
		StateManager.State.PLACE_BUILDING:
			var mouse_pos: Vector2 = get_parent().get_local_mouse_position()
			var dirt_layer: TileMapLayer = map_layer.get_child(1)
			var tile_pos: Vector2 = dirt_layer.local_to_map(mouse_pos)
			var world_pos: Vector2 = dirt_layer.map_to_local(tile_pos)
